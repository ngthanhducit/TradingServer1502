...
{
    public class ClientConnectPort
    {
        #region CREATE INSTANCE
        public static ClientConnectPort _instance;
        public static ClientConnectPort Instance
        {
            get
            {
                if (ClientConnectPort._instance == null)
                    ClientConnectPort._instance = new ClientConnectPort();
        ...
        public void ClientPort(string value, string ipAddress)
        {
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public string StringClientPort(string value, string ipAddress)
        {
                ...
                case "ClientCancelRequest":
                    {
                        ...
                        bool cancelRequest = TradingServer.Business.Market.marketInstance.RemoveNJ4XTicket(investorID, newRequestDealer.Request.Symbol.Name);
                ...
                case "ChangePrimaryPassword":
                    {
                        ...
                        bool changePwd = TradingServer.Facade.FacadeChangePasswordMT4(investorID, subParameter[1], subParameter[2]);
                ...
                case "GetData":
                    {
                        ...
                        //result = TradingServer.ClientFacade.FacadeWebGetData(investorID, subParameter[1], investorIndex);
                ...
                case "GetTopCandlesByTimeFrame":    //Port 0: web | 1: wcf
                    {
                        ...
                        string tempResult = QuotesServer.QuotesFacade.FacadeGetCandlesByTimeFrame(timeFrame, numGet, subParameter[3], time, port);

                        //EURUSD{1.3866000000000001{1.3863000000000001{1.3872{1.3861000000000001{275.{03/14/2014 05:00}
                        //string cmdAPI = subValue[0] + "$" + timeFrame + "{" + numGet + "{" + time + "{" + subParameter[3];
                        //string resultAPI = Business.Market.InstanceSocket.SendSocket(cmdAPI);
                        //result += Model.BuildCommand.Instance.MapStringToStrCandle(resultAPI);
                ...
                case "GetCandlesByTime":
                    {
                        ...
                        //string cmd = subValue[0] + "$" + timeFrame + "{" + time + "{" + subParameter[2];
                ...
                case "GetOpenPriceByTimeFrame":
                    {
                        ...
                        //string _tempResult = QuotesServer.QuotesFacade.FacadeGetOpenPriceByTimeFrame(timeFrame, numGet, listSymbol, time, port);
                        ...
                        if (listSymbol != null && listSymbol.Count > 0)
                        {
                            for (int i = 0; i < listSymbol.Count; i++)
                            {
                                ...
                                //string cmd = "GetTopCandlesByTimeFrame$" + timeFrame + "{" + numGet + "{" + time + "{" + listSymbol[i];
                                //string resultAPI = Business.Market.InstanceSocket.SendSocket(cmd);
                                //string tempResultAPI = Model.BuildCommand.Instance.MapStringToOpenPrice(resultAPI);
                                //result += listSymbol[i] + '♦';
                                //result += tempResultAPI + '▼';
                ...
                case "GetOnlineCandles":
                    {
                        ...
                        //GetOnlineCandles$EURUSD{1.3962000000000001{1.3952{1.3965000000000001{1.3952{100.{03/13/2014 13:00}
                        result += TradingServer.ClientFacade.FacadeGetCandleOnline(timeFrame, subParameter[1], port);

                        //string cmd = subValue[0] + "$" + timeFrame + "{" + subParameter[1];
                        //string resultAPI = Business.Market.InstanceSocket.SendSocket(cmd);
                        //result = Model.BuildCommand.Instance.MapStringToCandle(resultAPI);
                    }
                    break;
                #endregion
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public List<string> ClientPorts(string value, string ipAddress)
        {
                ...
                case "ClientLogin":
                    {
                        ...
                        {
                            if (Business.Market.IsConnectMT4)
                                objInvestor = TradingServer.ClientFacade.FacadeLoginMT4(subParameter[0].Trim(), subParameter[1].Trim(), ipAddress);
                            else
                                objInvestor = TradingServer.ClientFacade.FacadeNewLogin(subParameter[0], pwd, index, ipAddress, null);
                        ...
                        {
                            string message = subValue[0] + "$MOFF2367980";
                            result.Add(message);
                ...
                case "GetCandlesByDate":
                    {
                        ...
                        if (subParameter.Length == 7)
                        {
                            ...
                            //Get Candles By Day in server et5
                            //string cmd = subValue[0] + "$" + subParameter[0] + "|" + time;
                            //string resultAPI = Business.Market.InstanceSocket.SendSocket(cmd);
                            //List<string> tempResult = Model.BuildCommand.Instance.MapStringToCandles(resultAPI);

                            //Get Candles By Day in Manager API
                            //List<string> tempResult = Business.Market.marketInstance.GetCandlesByDate(subParameter[0] + "|" + time);

                            //if (tempResult != null && tempResult.Count > 0)
                            //{
                            //    int countTemp = tempResult.Count;
                            //    for (int i = 0; i < countTemp; i++)
                            //    {
                            //        if (!string.IsNullOrEmpty(tempResult[i]))
                            //        {
                            //            string message = subValue[0] + "$" + tempResult[i];
                            //            result.Add(message);
                            //        }
                            //    }
                            //}
                            ...
                            //Get Candles By Day in database et5
        ...
        /// <returns></returns>
        public string StringBrokerPort(string value, string ipAddress, string code)
        {
            string result = string.Empty;
            string[] subValue = value.Split('$');

            result = TradingServer.Business.Market.marketInstance.ExtractCommandServer(value, ipAddress, code);
        ...
        /// <param name="code"></param>
        /// <returns></returns>
        public List<string> ListStringBrokerPort(string value, string ipAddress, string code)
        {
            List<string> result = new List<string>();
            string[] subValue = value.Split('$');

            result = TradingServer.Business.Market.marketInstance.ExtractServerCommand(value, ipAddress, code);
